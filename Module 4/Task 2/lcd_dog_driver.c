...
int putchar(char c)
{
  if (index < NUMCHARS) 
    ...
    dsp_buff_2[index++ - NUMCHARS] = c;
#endif
  
 #ifdef LINE_3_ON
  else if (index < NUMCHARS * 3)
    dsp_buff_3[index++ - NUMCHARS * 2] = c;
#endif
...
void init_lcd_dog(void)
{
  ...
  __delay_cycles(40000 * clk_speed);    //startup delay
  
  lcd_spi_transmit_CMD(0x39);           //send function set #1
  __delay_cycles(30 * clk_speed);
  ...
  __delay_cycles(30 * clk_speed);       //power control

  lcd_spi_transmit_CMD(0x6C);           //follower mode
  __delay_cycles(40000 * clk_speed);     
